...
export const streamTokenProvider = async () => {
  try {
    ...
    // Add small negative skew to avoid "token used before issued" when client/server clocks differ
    const nowInSeconds = Math.floor(Date.now() / 1000);
    const issuedAt = nowInSeconds - 90; // 90s clock skew tolerance
    const expiresAt = nowInSeconds + 60 * 60; // 1 hour expiry
    ...
    console.log("Token iat:", issuedAt, "exp:", expiresAt);
